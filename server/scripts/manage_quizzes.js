...
const AIProviderFactory_1 = require("../src/ai/AIProviderFactory");
// Load env vars
...
async function main() {
    ...
    try {
        const provider = AIProviderFactory_1.AIProviderFactory.getProvider();
...
async function deleteSubject() {
    ...
    const { subjectId } = await inquirer_1.default.prompt([
        {
            type: 'list',
            name: 'subjectId',
    ...
    if (confirm) {
        await Question_1.Question.deleteMany({ subjectId });
        await Subject_1.Subject.findByIdAndDelete(subjectId);
...
async function deleteQuestions() {
    ...
    const questions = await Question_1.Question.find({ subjectId });
    if (questions.length === 0) {
        console.log('No questions in this subject.');
        return;
    }
    const { questionIds } = await inquirer_1.default.prompt([
        {
            type: 'checkbox',
            name: 'questionIds',
    ...
    if (confirm) {
        const result = await Question_1.Question.deleteMany({ _id: { $in: questionIds } });
        console.log(`✅ Deleted ${result.deletedCount} questions.`);
    }
}
async function createSubjectAndGenerate() {
    const { nameEn, nameHe, descEn } = await inquirer_1.default.prompt([
...
async function generateQuestionsFlow(subject) {
    // 1. Collect Inputs
...
async function collectInputs() {
    const { count, sourcesInput, subtopicsInput, difficultyInput } = await inquirer_1.default.prompt([
        {
            type: 'number',
            name: 'count',
    ...
    const sources = sourcesInput.split(',').map((s) => s.trim()).filter((s) => s.length > 0);
    const subtopics = subtopicsInput ? subtopicsInput.split(',').map((s) => s.trim()).filter((s) => s.length > 0) : [];
    return { count, sources, subtopics, difficulty: difficultyInput };
}
function constructPrompt(subject, inputs) {
    ...
    if (subject.description?.en)
        prompt += `Description: ${subject.description.en}\n`;
    prompt += `\nSTRICT REQUIREMENTS:\n`;
...
async function handleManualGeneration(prompt, subject) {
    console.log('\n[GENERATED PROMPT]');
    console.log('---------------------------------------------------');
    console.log(prompt);
    console.log('---------------------------------------------------');
    ...
    const { imported } = await inquirer_1.default.prompt([
    ...
    try {
        const content = fs_1.default.readFileSync(filePath.trim(), 'utf-8');
...
async function handleAutomaticGeneration(prompt, subject, count) {
    const provider = AIProviderFactory_1.AIProviderFactory.getProvider();
    ...
    try {
        ...
        const questions = await provider.generateFromPrompt(prompt);
        await saveQuestions(subject, questions);
    }
    catch (error) {
...
async function saveQuestions(subject, questions) {
    ...
    const validQuestions = questions.map(q => ({
        ...
        options: q.options.map((o) => o.text.en ? o : { text: { en: o.text, he: '' } })
    }));
    const result = await Question_1.Question.insertMany(validQuestions);
    console.log(`✅ Successfully added ${result.length} questions.`);
    await generateReport(subject, validQuestions);
}
async function generateReport(subject, questions) {
...
`;
    ...
    await subject.save();
    // Also save to file
    const reportPath = path_1.default.join(__dirname, `report_${subject._id}_${Date.now()}.md`);
